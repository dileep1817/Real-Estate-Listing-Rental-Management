...
export default function TenantProfile() {
  ...
  const email = user?.email || 'tenant@example.com'
  const [name, setName] = useState(user?.name || 'Tenant')
  ...
  const requests = getRequests().filter(r => (r.tenantEmail || 'tenant@example.com') === email)
  const total = requests.length
  const pending = requests.filter(r => r.status === 'pending')
  const accepted = requests.filter(r => r.status === 'accepted')
  const rejected = requests.filter(r => r.status === 'rejected')
  // filtered set for summary click-through
  const filteredRequests = (
    summaryFilter === 'all' ? requests :
    summaryFilter === 'pending' ? pending :
    summaryFilter === 'accepted' ? accepted :
    summaryFilter === 'rejected' ? rejected :
    []
  )

  // Per-listing breakdown for this tenant
  const byListing = (() => {
    const map = new Map()
    for (const r of requests) {
      const key = r.listing?.title || `Listing #${r.listing?.id || '—'}`
      const curr = map.get(key) || { total: 0, pending: 0, accepted: 0, rejected: 0 }
      curr.total += 1
      if (r.status === 'pending') curr.pending += 1
      if (r.status === 'accepted') curr.accepted += 1
      if (r.status === 'rejected') curr.rejected += 1
      map.set(key, curr)
    }
            ...
            <table className="table" style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                ...
                <tr onClick={() => setSummaryFilter('pending')} style={{ cursor: 'pointer' }}>
                  <td>Pending</td><td style={{ textAlign: 'right' }}>{pending.length}</td>
                </tr>
                ...
                <tr onClick={() => setSummaryFilter('rejected')} style={{ cursor: 'pointer' }}>
                  <td>Rejected</td><td style={{ textAlign: 'right' }}>{rejected.length}</td>
                </tr>
              </tbody>
            </table>
          </div>
            ...
            {filteredRequests.map(r => (
              <div key={r.id} className="card stack">
                ...
                <div className="row" style={{ gap: 8, justifyContent: 'space-between' }}>
                  <span className="muted">Owner: {r.listing?.ownerName || 'Owner'}</span>
                  {r.status === 'pending' && (
